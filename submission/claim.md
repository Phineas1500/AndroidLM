...
- Model: Qwen3.6-35B-A3B at 2-bit (12.3GB). Only 3B parameters are active per token, so the
  engine streams the experts each token needs from flash into a 2GB cache (BigMoeOnEdge on
  llama.cpp, plus two engine patches of ours). About 4.9GB of RAM in use; 3-4.4 tokens/s.
...
- Real timings on the phone: first words after 30-35 s for well-known subjects and 2-2.5 min
  for obscure ones; a complete cited answer in 2.5-7 min.
